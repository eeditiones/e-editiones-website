...
`tei-publisher-lib` 3.0.0 also introduces two new extensions to the processing model, namely *processing modes* and *parameter setting*. Read more about this in the [separate announcement](/posts/tei-publisher-lib-3/).
...
Please note that this feature requires an external service. If you would like to give it a try, please consult the [corresponding article](/posts/names-sell-named-entity-recognition-in-tei-publisher/).
...
* `tei-publisher-lib` 3.x.x reassigns the `print` output mode to generate HTML output targeted at print. In older version `print` was defined as an alias for `fo`. You may therefore have to change your ODDs if you used the fo mode. A more elaborate explanation of the changes is presented in this [e-editiones article](/posts/tei-publisher-lib-3/).